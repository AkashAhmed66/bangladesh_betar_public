  ...
  return (
    <div className="mx-auto w-full max-w-[1540px] px-4 pb-20 pt-8 sm:px-7 sm:pt-12 lg:px-10">
      <div className="max-w-3xl">
        ...
        <h1 className="mt-2 font-display text-4xl font-bold tracking-[-0.045em] sm:text-5xl">{t("watch.browseCategories")}</h1>
        <p className="mt-3 text-sm leading-relaxed text-ink-soft sm:text-base">{t("watch.browseCategoriesDescription")}</p>
      </div>

      {categoriesLoading ? (
        <div className="grid min-h-[24rem] place-items-center"><div className="flex items-center gap-3 text-sm font-bold text-ink-soft"><LoaderCircle className="size-5 animate-spin text-[var(--portal-color)]" /> {t("watch.loadingCategories")}</div></div>
      ) : (
        <div className="mt-9 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((category, index) => {
            const matching = shows.filter((show) => show.category === category.value);
            const label = localizedText(category as unknown as Record<string, unknown>, "label", locale);
            ...
            return (
              <Link key={category.slug} href={href} className="group relative min-h-60 overflow-hidden rounded-panel p-6 text-white" style={{ backgroundColor: COLORS[index % COLORS.length] }}>
                ...
                <div className="relative flex h-full min-h-48 flex-col justify-end">
                  ...
                  <h2 className="mt-4 font-display text-3xl font-bold">{label}</h2>
                  <div className="mt-2 flex items-center justify-between text-sm font-bold text-white/75"><span>{t(matching.length === 1 ? "common.show" : "common.shows", { count: matching.length })}</span><span className="inline-flex items-center gap-1 transition group-hover:gap-2">{t("common.explore")} <ArrowRight className="size-4" /></span></div>
                </div>
              </Link>
            );
          })}
