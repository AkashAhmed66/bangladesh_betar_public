...
export default function Comments({
  ...
  const submit = async () => {
    ...
    if (!token) {
      openLoginPrompt(t("comments.login"));
      return;
    }
    setBusy(true);
    try {
      ...
      toast((e as { firstError?: string }).firstError ?? t("comments.submitFailed"), "error");
    } finally {
      setBusy(false);
    }
  };
  ...
  const remove = async (id: number) => {
    try {
      ...
      toast(t("comments.deleteFailed"), "error");
    }
  };
    ...
    <section>
      <div className="mb-4 flex flex-wrap items-center gap-x-3 gap-y-1">
        <h2 className="flex items-center gap-2 font-display text-xl font-bold">
          <MessageSquare className="size-5 text-accent" /> {t("comments.title")}
        </h2>
        ...
        {avgRating != null && ratingCount > 0 && (
          <span className="flex items-center gap-1.5 text-sm text-ink-soft">
            ...
            {avgRating.toFixed(1)} · {t(ratingCount === 1 ? "comments.rating" : "comments.ratings", { count: ratingCount })}
          </span>
        )}
      </div>
      ...
      <div className="mb-6 flex items-start gap-3">
        ...
        <div className="flex-1">
          <div className="mb-2 flex items-center gap-2">
            <StarRating
              value={pendingRating}
              onChange={(v) => (token ? setPendingRating(v) : openLoginPrompt(t("comments.loginRate")))}
              size="size-5"
            />
            {pendingRating > 0 && (
              <button
          ...
          {allowComments ? (
            <textarea
              ...
              rows={2}
              maxLength={2000}
              placeholder={token ? t("comments.placeholder") : t("comments.guestPlaceholder")}
              onFocus={() => !token && openLoginPrompt(t("comments.login"))}
            ...
            <p className="text-sm text-ink-mute">{t("comments.disabled")}</p>
          )}
          <div className="mt-2 flex justify-end">
            ...
            >
              <Send className="size-3.5" /> {t("comments.post")}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        {isLoading && <p className="text-sm text-ink-mute">{t("comments.loading")}</p>}
    ...
    <div className="group flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1 rounded-card bg-raised/60 px-4 py-3">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-sm font-semibold">{c.author ?? t("comments.listener")}</p>
          {c.rating != null && <StarRating value={c.rating} size="size-3" />}
          <p className="text-[11px] text-ink-mute">{timeAgo(c.created_at, locale)}</p>
          {c.is_mine && (
            <button
              onClick={onDelete}
