...
export default function SupportPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast(err instanceof ApiError ? err.firstError : t("supportPage.failed"), "error");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-8">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-tight sm:text-3xl">{t("supportPage.title")}</h1>
      ...
      <div className="rounded-panel border border-edge bg-raised/50 p-4 sm:p-6">
        ...
        <div className="mb-5 grid w-full grid-cols-2 rounded-2xl border border-edge-strong p-1 sm:inline-flex sm:w-auto sm:rounded-full">
          ...
          >
            <MessageSquareHeart className="size-4" /> {t("supportPage.send")}
          </button>
          <button
            type="button"
          ...
          >
            <Bug className="size-4" /> {t("supportPage.problem")}
          </button>
        </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {FEEDBACK_CATEGORIES.map((c) => (
                  <button key={c.value} type="button" onClick={() => setCategory(c.value)} className={chip(category === c.value)}>
                    {t(`supportPage.${c.value}`)}
                  </button>
                ))}
              </div>
              <input className={input} placeholder={t("supportPage.subject")} value={subject} onChange={(e) => setSubject(e.target.value)} maxLength={255} />
              <textarea
                className={`${input} resize-none`}
                rows={4}
                required
                maxLength={2000}
                placeholder={t("supportPage.message")}
                value={message}
            ...
            <>
              <div className="flex flex-wrap gap-2">
                {ISSUE_TYPES.map((issue) => (
                  <button key={issue.value} type="button" onClick={() => setIssueType(issue.value)} className={chip(issueType === issue.value)}>
                    {t(`supportPage.${({ broken_audio: "brokenAudio", wrong_metadata: "wrongMetadata", inappropriate: "inappropriate", other: "other" } as const)[issue.value]}`)}
                  </button>
                ))}
              </div>
              <textarea
                ...
                required
                maxLength={1000}
                placeholder={t("supportPage.happened")}
                value={description}
          ...
          >
            ...
            {mode === "feedback" ? t("supportPage.send") : t("supportPage.submit")}
          </button>
        </form>
      ...
      <section>
        <div className="mb-3 flex items-center gap-2">
          ...
          <h2 className="font-display text-lg font-bold">{t("supportPage.submissions")}</h2>
        </div>

        {!token ? (
          <div className="flex flex-col items-start gap-3 rounded-panel border border-edge bg-raised/50 p-4 sm:p-6">
            <p className="flex items-center gap-2 text-sm text-ink-soft">
              <Lock className="size-4" /> {t("supportPage.loginStatus")}
            </p>
          ...
          <div className="rounded-panel border border-edge bg-raised/50 p-6 text-sm text-ink-mute">{t("supportPage.loading")}</div>
          ...
          <div className="rounded-panel border border-edge bg-raised/50 p-8 text-center">
            ...
            <p className="mt-2 text-sm font-semibold">{t("supportPage.empty")}</p>
            <p className="mt-1 text-xs text-ink-mute">{t("supportPage.emptyDescription")}</p>
          </div>
        ) : (
                ...
                <li key={s.id} className="rounded-panel border border-edge bg-raised/50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-bold ${STATUS_STYLE[s.status]}`}>
                      {t(`supportPage.${({ new: "new", in_progress: "inProgress", resolved: "resolved", dismissed: "dismissed" } as const)[s.status]}`)}
                    </span>
                  </div>
                  ...
                  {s.target?.label && (
                    <p className="mt-1 text-xs text-ink-mute">
                      {t("supportPage.about")} <span className="font-medium text-ink-soft">{s.target.label}</span>
                    </p>
                  )}

                  {s.resolution_notes && (
                    <div className="mt-3 rounded-card border border-edge bg-elev/60 p-3">
                      <p className="text-[11px] font-bold uppercase tracking-wide text-ink-mute">{t("supportPage.response")}</p>
                      <p className="mt-1 text-sm text-ink-soft">{s.resolution_notes}</p>
                    </div>
                  )}

                  <p className="mt-2 text-[11px] text-ink-mute">{t("supportPage.submitted", { date: fmtDate(s.created_at) })}</p>
                </li>
              );
            })}
          </ul>
