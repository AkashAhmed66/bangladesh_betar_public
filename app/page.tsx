...
function greetingKey(): string {
  ...
  if (hour < 5) return "listen.lateNight";
  if (hour < 12) return "listen.goodMorning";
  if (hour < 17) return "listen.goodAfternoon";
  return "listen.goodEvening";
}
...
export default function HomePage() {
  ...
  const heroTitle = banner
    ...
    : heroItem
      ...
      : t("listen.heroFallback");
  const heroSubtitle = banner
    ? (locale === "bn" && banner.subtitle_bn ? banner.subtitle_bn : banner.subtitle) ?? t("listen.heroDescription")
    : t("listen.heroDescription");
    ...
    <div className="flex flex-col gap-12 sm:gap-16">
      {isLoading ? (
        ...
        <section className="portal-full-bleed relative -mt-5 overflow-hidden border-b border-edge bg-raised sm:-mt-7">
          ...
          <div className="relative mx-auto grid min-h-[34rem] max-w-[1540px] items-center gap-10 px-5 py-12 sm:px-10 lg:min-h-[38rem] lg:grid-cols-[28rem_1fr] lg:gap-16 lg:py-16">
            ...
            <div className="min-w-0 max-w-3xl">
              <p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.2em] text-[var(--portal-color)]"><Sparkles className="size-4" /> {t("listen.featuredOn", { brand: BRAND.shortName })}</p>
              ...
              <div className="mt-7 flex flex-wrap gap-3">
                {heroItem && (
                  <button type="button" onClick={() => void playItem(heroItem)} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[var(--portal-color)] px-6 text-sm font-black text-white transition hover:scale-[1.02]"><Play className="size-4 fill-current" /> {t("listen.listenNow")}</button>
                )}
                <Link href={heroItem ? itemHref(heroItem) : banner?.target_value || "/browse"} className="inline-flex min-h-12 items-center gap-2 rounded-full border border-edge-strong bg-elev/50 px-6 text-sm font-black backdrop-blur transition hover:bg-highlight">{t("listen.moreInfo")} <ArrowRight className="size-4" /></Link>
              </div>
            </div>
          </div>
          ...
          <div className="relative mx-auto flex max-w-[1540px] items-center gap-3 px-5 pb-8 sm:px-10">
            ...
            <button type="button" onClick={() => setSlide((value) => (value + 1) % Math.max(1, slideCount))} className="grid size-11 shrink-0 place-items-center rounded-full border border-edge bg-elev/70 text-ink-soft backdrop-blur" aria-label={t("listen.nextFeature")}><ChevronRight className="size-5" /></button>
          </div>
        </section>
      )}
            ...
            {live!.data.map((channel) => (
              <Link href={`/live/${channel.id}`} key={channel.id} className="group flex w-72 shrink-0 items-center gap-4 rounded-panel border border-edge bg-raised p-4 transition hover:-translate-y-0.5 hover:border-edge-strong">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 clamp-1 text-xs text-ink-mute">{channel.is_live ? channel.session_title || t("listen.liveNow") : t("listen.offlineNow")}</p>
                  <span className="mt-3 inline-flex items-center gap-1.5 text-xs font-black text-[var(--portal-color)]"><Radio className="size-3.5" /> {t("listen.openStation")}</span>
                </div>
              </Link>
            ))}
          </div>
      ...
      {resumeItems.length > 0 && (
        <section>
          <SectionHeading title={<span className="editorial-rule">{t(greetingKey())}{user ? `, ${user.name.split(" ")[0]}` : ""}</span>} />
              ...
              return (
                <div key={recording.id} className="group relative flex items-center gap-3 overflow-hidden rounded-card border border-edge bg-raised transition hover:border-edge-strong hover:bg-highlight">
                  ...
                  <div className="min-w-0 flex-1 py-2 pr-2">
                    ...
                    <p className="mt-1 text-xs text-ink-mute">{formatDuration(progress_seconds)} / {formatDuration(recording.duration_seconds)} · {t("listen.resume")}</p>
                  ...
                  {track && <div className="pr-3"><PlayCircle size="size-10" icon="size-4" onClick={() => playTrack(track, progress_seconds)} label={t("listen.resumeTitle", { title: displayTitle(recording, locale) })} /></div>}
                </div>
              );
            })}
          </div>
      ...
      {forYou && forYou.data.length > 0 && (
        <SectionRow title={<span className="editorial-rule flex items-center gap-2">{forYou.personalized ? t("listen.madeForYou") : t("listen.popularNow")}{forYou.personalized && <Sparkles className="size-4 text-[var(--portal-color)]" />}</span>} items={forYou.data} />
      )}

      {isLoading && <><SectionRow title={t("listen.loading")} loading /><SectionRow title={t("listen.loading")} loading /></>}
      ...
      <section className="portal-tint-panel overflow-hidden rounded-panel">
        <div className="grid gap-6 p-7 sm:p-10 md:grid-cols-[1fr_auto] md:items-center">
          <div>
            <p className="portal-tint-eyebrow flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em]"><Headphones className="size-4" /> {t("listen.oneAccount")}</p>
            <h2 className="mt-3 font-display text-3xl font-bold tracking-tight sm:text-4xl">{t("listen.carryStories")}</h2>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-ink-soft">{t("listen.accountDescription")}</p>
          </div>
          <Link href={user ? "/account" : "/login"} className="portal-tint-action inline-flex min-h-12 w-fit items-center gap-2 rounded-full px-6 text-sm font-black transition">{user ? t("listen.openAccount") : t("common.login")} <ArrowRight className="size-4" /></Link>
        </div>
      </section>
    </div>
  );
