  ...
  return (
    <section id={id} className="scroll-mt-32">
      <div className="mb-5 flex items-center justify-between gap-4">
        ...
        <Link href={href} className="hidden items-center gap-1 text-xs font-black uppercase tracking-[0.12em] text-ink-mute hover:text-ink sm:flex">View all <ArrowRight className="size-3.5" /></Link>
      </div>
...
export default function WatchPage() {
  ...
  if (isLoading) return <div className="grid min-h-[30rem] place-items-center"><div className="flex items-center gap-3 text-sm font-bold text-ink-soft"><LoaderCircle className="size-5 animate-spin text-[var(--portal-color)]" /> Loading Watch…</div></div>;
  if (error || shows.length === 0) {
    return <div className="mx-auto grid min-h-[30rem] max-w-xl place-items-center px-5 text-center"><div><AlertCircle className="mx-auto size-9 text-[var(--portal-color)]" /><h1 className="mt-4 font-display text-3xl font-bold">Watch is unavailable</h1><p className="mt-2 text-sm text-ink-soft">Published shows could not be loaded. Please try again shortly.</p></div></div>;
  }
      ...
      <section id="drama" className="relative min-h-[32rem] overflow-hidden bg-black text-white scroll-mt-32 sm:min-h-[38rem] lg:min-h-[43rem]">
        ...
        <div className="relative mx-auto flex min-h-[32rem] max-w-[1540px] items-end px-4 pb-12 pt-24 sm:min-h-[38rem] sm:px-8 sm:pb-16 lg:min-h-[43rem] lg:px-12">
          <div className="min-w-0 max-w-2xl">
            <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[0.2em] text-[#75e3df]"><Sparkles className="size-4" /> {featured.eyebrow ?? featured.category}</p>
            ...
            <p className="mt-5 text-xs font-bold text-white/55">{featured.year ?? "New"} · {featured.rating ?? "Not rated"} · {featured.category} · {featured.episodes_count ?? featured.episodes.length} episodes</p>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto flex w-full max-w-[1540px] flex-col gap-14 px-4 pt-10 sm:px-7 sm:pt-12 lg:px-10">
        <section id="live" className="overflow-hidden rounded-panel border border-edge bg-raised scroll-mt-32">
          <div className="grid items-center gap-6 p-6 sm:p-8 md:grid-cols-[auto_1fr_auto]">
            ...
            <div><p className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-danger"><span className="size-2 animate-pulse rounded-full bg-danger" /> Live television</p><h2 className="mt-1 font-display text-2xl font-bold">Betar News 24</h2><p className="mt-1 text-sm text-ink-soft">Continuous public news, parliament, weather and national events.</p></div>
        ...
        <Shelf id="trending" title="Trending now" shows={shows.slice(0, 6)} href="/watch/categories" ranked />
        <Shelf id="documentary" title="Documentaries that take you further" shows={uniqueShows([...docs, ...shows.slice(0, 2)])} href="/watch/category/documentary" />
        <Shelf id="culture" title="Culture, music & performance" shows={uniqueShows([...culture, ...shows.filter((show) => show.category === "Drama").slice(0, 2)])} href="/watch/category/culture" />
        <Shelf id="kids" title="For young explorers" shows={uniqueShows([...kids, ...shows.slice(0, 2)])} href="/watch/category/kids" />
        ...
        <section id="categories" className="scroll-mt-32">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[["Drama", "#7c2846"], ["Documentary", "#146b62"], ["Culture", "#8a4a16"], ["Kids", "#315fa8"]].map(([label, color]) => (
              <Link key={label} href={`/watch/category/${label.toLowerCase()}`} className="group relative min-h-32 overflow-hidden rounded-panel p-5 text-white" style={{ background: color }}><span className="absolute -bottom-10 -right-8 size-28 rounded-full border-[16px] border-white/10 transition group-hover:scale-125" /><span className="relative font-display text-xl font-bold">{label}</span></Link>
            ))}
          </div>
        </section>
      </div>
