  ...
  if (hydrated && !token) {
    return (
      <EmptyState
        ...
        title={t("libraryPages.librarySignInTitle")}
        subtitle={t("libraryPages.librarySignInDescription")}
        action={
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-3xl font-bold tracking-tight">{t("libraryPages.libraryTitle")}</h1>
        <button
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Link href="/favorites" className="flex items-center gap-4 rounded-panel bg-gradient-to-br from-accent/25 to-raised p-5 transition hover:from-accent/35">
          ...
          <div>
                <p className="font-display font-bold">{t("libraryPages.liked")}</p>
                <p className="text-xs text-ink-soft">{t("libraryPages.likedDescription")}</p>
          </div>
        </Link>
        <Link href="/history" className="flex items-center gap-4 rounded-panel bg-gradient-to-br from-highlight to-raised p-5 transition hover:from-edge-strong">
          ...
          <div>
                <p className="font-display font-bold">{t("libraryPages.history")}</p>
                <p className="text-xs text-ink-soft">{t("libraryPages.historyDescription")}</p>
          </div>
        </Link>
      </div>
      ...
      {/* Playlists */}
      <section>
        <SectionHeading title={t("shell.playlists")} />
        {isLoading ? (
      ...
      {/* Follows */}
      <section>
        <SectionHeading title={t("libraryPages.following")} />
        {followGroups.length === 0 ? (
          <EmptyState
            icon={<UserCheck className="size-10" />}
            title={t("libraryPages.notFollowing")}
            subtitle={t("libraryPages.notFollowingDescription")}
          />
        ) : (
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title={t("libraryPages.newPlaylist")}>
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            placeholder={t("libraryPages.titleBangla")}
            className="rounded-card border border-edge-strong bg-raised px-4 py-2.5 text-sm outline-none focus:border-accent"
          />
          <button
